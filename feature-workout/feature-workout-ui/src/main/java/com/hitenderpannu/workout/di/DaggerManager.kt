...
object DaggerManager {
    ...
    fun inject(newWorkoutActivity: NewWorkoutActivity) {
        if (workoutComponent == null) initializeWorkoutComponent(newWorkoutActivity.application as MainApplication)
        DaggerNewWorkoutComponent.builder()
            .workoutComponent(workoutComponent)
            .newWorkoutModule(NewWorkoutModule(newWorkoutActivity))
            .build().inject(newWorkoutActivity)
    }
}
